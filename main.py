...
def userhome():
    if not session.get('userlogin'):
        return render_template('userlogin.html'),401
    else:
...
def show_update_detail():
    ...
    if request.method=='POST':
        ...
        if("update" in request.form):
            return render_template('update_user_page.html',res = upd_res,fields = fields_upd, not_found=not_found)
...
def add_User():
    if not(session.get('login') or session.get('userlogin')):
        return redirect( url_for('home') )
